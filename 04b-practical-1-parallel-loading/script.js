document.addEventListener("DOMContentLoaded", async function () {
    ...
    const natureLayerPromise = createLayer("nature.json", map);

    // when you await a promise, JS will pause
    // at that line and return the value from the promise (function that takes a long time)
    const hdbLayer = await hdbLayerPromise;
    const mallLayer = await mallLayerPromise;
    const natureLayer = await natureLayerPromise;
