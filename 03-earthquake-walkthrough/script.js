...
function drawEarthquakeMarkers(layer, coordinates){ // name of coordinates does not have to match line 17
    for (let c of coordinates){
        const marker = L.marker(c.coordinates); // marker is also known as a layer 
        marker.bindPopup(`<h1>${c.title}</h1>`);
        marker.addTo(layer); // this function will add marker to the layer
    }
}
